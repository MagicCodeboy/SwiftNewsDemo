...
extension UILabel {
    ...
    func setSeparatedLinesFrom(_ attributedString: NSMutableAttributedString, hasImage: Bool) {
        ...
        let frame = CTFramesetterCreateFrame(frameSetter, CFRange.init(location: 0, length: attributedString.length), path, nil)
        //获取当前 frame 中的每一行的内容
        ...
        for (index, line) in lines.enumerated() {
            ...
            //当前的内容
            let currentAttributedString = NSMutableAttributedString.init(attributedString: attributedString.attributedSubstring(from: range))
    ...
    private func replaceContent(_ currentAttributedString: NSMutableAttributedString) {
        currentAttributedString.replaceCharacters(in: NSRange.init(location: currentAttributedString.length - 8, length: 8), with: NSAttributedString.init(string: "...全文\n", attributes: [.foregroundColor: UIColor.blueFontColor()]))
    }
}
...
extension UICollectionView {
    ...
    func ym_registerSupplementaryHeaderView<T: UICollectionReusableView>(reusableView: T.Type) where T:RegisterCellFromNib {
            //T 遵守了 RegisterCellOrNib 协议， 所以通过 T 就能取出identifier 这个属性
        if let nib = T.nib {
             register(nib, forSupplementaryViewOfKind: UICollectionElementKindSectionFooter, withReuseIdentifier: T.identifier)
        } else {
...
extension UIColor {
    ...
    class func globalRedColor() -> UIColor {
         return UIColor.init(r: 196, g: 73, b: 67)
    }
    //字体蓝色
    class func blueFontColor() -> UIColor {
        return UIColor.init(r: 72, g: 100, b: 149)
    }
    /// 背景灰色 132
