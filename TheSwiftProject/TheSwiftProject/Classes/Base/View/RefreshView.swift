...
class RefreshHeader: MJRefreshGifHeader {
    ...
    override func placeSubviews() {
        ...
        gifView.frame = CGRect.init(x: 0, y: 4, width: mj_w, height: 25)
        stateLabel.font = UIFont.systemFont(ofSize: 12)
        stateLabel.frame = CGRect.init(x: 0, y: 35, width: mj_w, height: 14)
    }
}
...
class RefreshAutoGifFooter: MJRefreshAutoGifFooter {
    ...
    override func prepare() {
        ...
        //遍历
        for index in 0..<8 {
            let image = UIImage.init(named: "sendloading_18x18_\(index)")
            images.append(image!)
        }
        //设置空闲状态的图片
        setImages(images, for: .refreshing)
