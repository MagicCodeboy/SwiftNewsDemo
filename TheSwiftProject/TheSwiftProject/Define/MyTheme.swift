...
struct MyThemeStyle {
    ...
    static func setupNavigationBarStyle(_ viewController: UIViewController, _ isNight: Bool){
        if isNight { //设置夜间主题
            ...
            viewController.navigationController?.navigationBar.setBackgroundImage(UIImage.init(named: "navigation_background_white_night"), for: .default)
            ...
            viewController.navigationController?.navigationBar.setBackgroundImage(UIImage.init(named: "navigation_background_white"), for: .default)
        }
    }
}
